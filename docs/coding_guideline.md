...
- Module boundaries should reflect this: a module that owns one responsibility is easier
  to test, name, and reason about than one that owns several.
...
- A rendering/UI module should not need to know how data is fetched or stored; a storage
  module should not need to know how data is displayed. Each side depends only on a shared
  data type, not on the other's implementation.
...
A common failure pattern: bad input accepted at the CLI level propagates silently through
several layers before producing a misleading error far from its actual cause. The fix is to
validate at the earliest possible boundary.

**Bad (before)**:

```rust
// cli.rs — no validation; bad input silently carried forward
pub fn into_config(self) -> Config {
    Config { path: self.path.unwrap_or(".".into()), ... }
}

// ... several layers later ...
// some deeply nested call fails with an error that doesn't mention the CLI flag at all
```
...
pub fn into_config(self) -> anyhow::Result<Config> {
    ...
    validate_input(&path)?;  // <-- rejects bad input right here
    Ok(Config { path, ... })
}

/// Pure function: easy to unit test independently of the rest of the stack.
pub fn validate_input(path: &Path) -> anyhow::Result<()> {
    if !path.exists() {
        return Err(anyhow::anyhow!(
            "{:?} does not exist. Pass a valid path.", path
        ));
    }
    Ok(())
}
...
- Functions that perform actions should use verbs: `parse_config`, `send_request`, `build_index`.
- Use consistent vocabulary across layers: pick one term per concept (e.g. always `item`,
  never a mix of `item`/`entry`/`record` for the same thing) and use it everywhere.

### Layer-Specific Naming

| Layer | Vocabulary |
|---|---|
| I/O (files, network) | `read`, `write`, `load`, `save` |
| Protocol / message handling | `send`, `receive`, `request`, `response`, `notification` |
| Domain / Application | `build`, `render`, `search`, `filter`, `match` |
...
- **Be consistent**: follow a uniform error-naming convention, e.g. prefix messages with
  the failing layer or operation (`io: ...`, `parse: ...`, `network: ...`) so the source of
  an error is clear at a glance.
...
// Good
"io: failed to read config file: unexpected EOF after 3 bytes"
```
